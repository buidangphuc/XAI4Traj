...
from .gru import GRUBlock, build_model
...
class GRUModel(Model):
    """Implementation of a GRU model using PyTorch."""
    ...
    def __init__(
        ...
        self.mlp_units = [64] if mlp_units is None else mlp_units
    ...
    def train(
        ...
        if cross_validation == 0:
            ...
            if model_path is None:
                model = self._get_model(n_classes, input_shape, mask=self.mask_value)
            else:
                model = self._get_model(n_classes, input_shape, mask=self.mask_value)
    ...
    def _reshape_input(self, x_data: np.ndarray) -> np.ndarray:
        """Reshapes the input data to be compatible with the GRU model."""
        # The GRU model handles 3D inputs (batch_size, seq_len, features)
        return x_data  # Already has shape (batch_size, seq_len, features)
